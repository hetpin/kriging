...
for small_item in list_data:
	index = index + 1
	f.write(str(small_item.id) + ' ' + small_item.file_name+' '+ small_item.name_tinh+ ' '+ small_item.name_tram + '\n' )

#IMPORTING
list_ = []
for item in list_data:
	list_.append((item.id, item.name_tram, item.name_tinh, item.lat, item.lon, item.alt, item.file_name))
con = None
try:
