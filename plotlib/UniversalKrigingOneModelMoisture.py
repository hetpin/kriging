#Simple Kringing Implementation for moiseture
import numpy as np
import time, os
import matplotlib
...
from Utility import moiseture_getSateliteDeriMap,moiseture_get_sate_lite_value,reIndex
...
def generateGlobalMatrix( date_from, date_to, list_all_station):
	...
	for i in xrange(0,meteo_size):
		...
		list_all = zip(*item)
		#1 for lat, 2 for lon, 3 for temp, 4 for humid, 5 for rain
		list_temp = list_all[4]
		#print list_temp
		list_data.append(list_temp)
		mean_item = (list_all[1][0],list_all[2][0], sum(list_all[4])/len(list_all[4]))
		#print len(list_all[4]), list_all[4]
...
def start(date_from_changed, date_to_changed, isCrossValidation):
	f = open('Moiseture_log_universal.txt', 'w')
	#INTERPOLATE
	global date_to
	global date_from
	...
	sate_lite_deri, z_sate_to_surface = moiseture_getSateliteDeriMap()
	print z_sate_to_surface
	global sate_lite_value
	sate_lite_value = moiseture_get_sate_lite_value(date_to, z_sate_to_surface)
