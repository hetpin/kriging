...
from get_dif import getAStationRain
import math
import os.path
...
for x in xrange(1,size+1):
	list_station_all.append(getAStationRain(str(x)))
	#list_index.append(x)
...
for x in xrange(0,32):
	...
	for y in xrange(0,size_h):
		if (list_h[y] > d*x) and (list_h[y] < d*(x+1)):
			if not math.isnan(list_value[y]):			
				total = total +1
				mean = mean + list_value[y]
				print mean
	if total >0:
		print str(mean), ' ', str(total), ' one period ', str(x) 
		list_period_value.append(mean/total)
		list_period_h.append(d*x)
	else:
		print str(mean), ' ', str(total), ' NOO period ', str(x) 		
...
plt.ylabel('r')
plt.savefig(os.path.splitext(os.path.basename('Rain_correlation_all'))[0] + '.png')
plt.show()
